...
/**
 * this takes all inbound AMQP messages, passes them through the echo
 * service and returns.
 */
@Configuration
public class AmqpIntegration {
	...
	public IntegrationFlow amqpReplyFlow(ConnectionFactory rabbitConnectionFactory,
		...
		return IntegrationFlows.from(
				Amqp.inboundGateway(rabbitConnectionFactory, this.echoQueueAndExchangeName)
			).transform(String.class, echoService::echo).get();
	}

	@Bean
	public CommandLineRunner client(RabbitTemplate template) {
		return args -> Arrays.asList("Josh", "Phil").forEach(n ->
			System.out.println("template response: " + template.convertSendAndReceive(this.echoQueueAndExchangeName, n)));
	}
}
